...
| v2.1 (d) | CE + Oversample +RA=5 | 78.85% | 0.5703 | 0.7895 | 0.6281 |
| v2.1 (e) | CE + Oversample +基础增强 | 81.77% | 0.5842 | 0.8130 | **0.6604** |
...
所有变体的 Macro F1 均低于 CE 基线（最高 0.5989 vs 0.6464）。后续追加的 CE + 过采样实验（v2.1 d/e）也未能超越 CE 基线：
- CE + 过采样 + RA=5 效果最差（0.5703），说明 RandAugment 在没有 Focal Loss 保护时可能破坏判别特征
- CE + 过采样 + 基础增强 Val Best 创新高（0.6604），但 Test Macro F1 仅 0.5842，验证/测试差距大

### 最终结论

**v2.1 (c) — Focal Loss γ=1.0 + 过采样 + RandAugment（关 MixUp）** 是综合最优配置：
- Accuracy 81.85%（所有版本最高）
- Weighted F1 0.8142（所有版本最高）
- Macro F1 0.5989（Focal Loss 系列中最高）
- 尾部类 Angiectasia 从 F1=0 提升至 0.6667，Colorectal cancer 0.56→0.70

CE 基线的 Macro F1（0.6464）虽仍最高，但牺牲了 Accuracy（仅 71.44%），在实际应用中多数类分类能力不足。

### 后续建议

1. **数据层面** — 3 个 F1=0 的类测试样本仅 1-2 张，无法可靠评估；收集更多尾部类数据是提升 Macro F1 最直接的路径
2. **类别合并** — 对 Erythema/Angiectasia 等样本极少且临床意义相近的类，考虑合并
3. **集成方法** — 多折交叉验证后集成推理，可能进一步提升稳定性
